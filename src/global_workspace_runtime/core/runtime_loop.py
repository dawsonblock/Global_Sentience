...
from .types import ObservationPacket, MemoryEpisode, ThoughtCandidate, clamp01
from ..modulation import update_internal_state, update_resource_state, infer_resonance_tags
from ..memory.retrieval import retrieve_state_weighted
from ..cognition import LLMAdapter, AnalyticStream, AssociativeStream, CreativeAssociativeStream, ConceptualBlender, SelfModelStream, InterhemisphericBridge, Critic, Planner, ReactiveLayer, determine_candidate_budget, prescreen_candidates
from ..workspace import GlobalWorkspace, candidate_to_capsule


class GlobalWorkspaceRuntime:
    def __init__(self, config: RuntimeConfig | None = None, state: RuntimeState | None = None) -> None:
        ...
        trace_path = Path(self.config.trace_dir) / f"trace-{int(time.time())}.jsonl"
    ...
    def observe(self, text: str, source: str = "user") -> ObservationPacket:
        return ObservationPacket(f"obs-{uuid.uuid4().hex[:10]}", time.time(), source, text, {}, {"runtime": "GlobalWorkspaceRuntime"})

    def apply_world_feedback(self, outcome: dict[str, Any]) -> None:
        """Feed closed-world outcome metrics back into the runtime state.

        SimWorld is an evaluator, not just a logger.  This method lets outcome
        deltas adjust internal pressure, resource pressure, and somatic memory so
        repeated mistakes can influence later selections.
        """
        ...
        self.state.somatic_map.update(
            state,
            {
                "contradiction": max(0.0, 1.0 - total),
                "memory_conflict": max(0.0, 1.0 - uncertainty_resolution),
                "action_outcome": total,
            },
    ...
    def _signals_from_observation(self, obs: ObservationPacket) -> dict[str, float]:
        ...
        return {"risk": risk, "contradiction": contradiction, "ambiguity": ambiguity, "curiosity": curiosity, "prediction_error": prediction_error, "resource_pressure": 0.1, "user_feedback": 0.6, "action_outcome": 0.5, "memory_conflict": contradiction}
    ...
    def run_cycle(self, text: str, source: str = "user", force_slow: bool = False) -> dict[str, Any]:
        ...
        update_resource_state(self.state.resource_state, {"compute_usage": 0.03, "time_usage": 0.02, "memory_usage": 0.01, "context_usage": min(1.0, len(text)/4000)})
        ...
        self.state.somatic_map.update(
            state,
            signals,
            scratchpad_pressure=scratchpad_pressure,
            memory_pressure=signals.get("memory_conflict", 0.0),
        )
        state.validate()
        self.state.self_model.observe_telemetry(state.snapshot())
        self._emit(cycle_id, RuntimePhase.EVALUATE, "high_priority" if state.uncertainty > .5 or state.threat > .5 else "low_priority", "state_update", {"internal_state": state.snapshot(), "somatic": self.state.somatic_map.snapshot()})
        ...
        if self.config.long_term_archive_enabled:
            archive_hits = self.state.long_term_archive.query(text, limit=2)
            memory_context.extend(archive_hits)
        self._emit(cycle_id, RuntimePhase.RECALL, "urgent_retrieval" if state.threat > .6 else "normal_retrieval", "memory_context", {"count": len(memory_context), "items": memory_context})
        ...
        analytic = self.analytic.generate(workspace_packet, memory_context, state, max(2, budget//2))
        associative = self.associative.generate(workspace_packet, memory_context, state, max(2, budget//2))
        ...
        self_diag = self.self_stream.generate(workspace_packet, memory_context, state, 1)
        creative = None
        ...
        # Self-model diagnostics are telemetry, not user-facing actions.  They are
        # scored separately so tests can verify rejection/grounding, but they do
        # not enter workspace competition unless the planner has no alternatives.
        ...
        overflow_candidates = self.state.scratchpad.write_overflow(candidates, capacity=max(self.config.workspace_capacity, 2))
        ...
        scored = self.critic.score_many(
            [cap.candidate for cap in ws.shortlist if cap.candidate],
            state,
            memory_context,
            ws,
        )
        ...
        episode = MemoryEpisode(
            ...
            outcome_score=0.5,
            provenance={"observation_id": obs.observation_id, "trace_path": str(self.tracer.path), "archive_enabled": self.config.long_term_archive_enabled, "action_type": decision.action_type},
        )
