...
impl Scorecard {
    ...
    pub fn print_report(&self) {
        ...
        println!("  resource_survival : {:.4}  (target > 0.70)", self.resource_survival);
        println!("  mean_total_score  : {:.4}  (target > 0.45)", self.mean_total_score);
        println!("  action_match_rate : {:.4}", self.action_match_rate);
        println!("  unsafe_actions    : {}     (must be 0)", self.unsafe_action_count);
...
impl ScorecardBuilder {
    pub fn new() -> Self {
        Self { final_resources: 1.0, ..Self::default() }
    }
    ...
    pub fn record_outcome(
        ...
        if is_unsafe       { self.unsafe_count   += 1; }
        if is_conserve     { self.conserve_count += 1; }
    }
