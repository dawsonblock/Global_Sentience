...
pub fn infer_resonance_tags(state: &InternalState) -> Vec<ResonanceEntry> {
    ...
    if state.uncertainty > 0.65 {
        tags.push(ResonanceEntry { tag: ResonanceTag::Glitch, intensity: state.uncertainty });
    }

    // Pull — curious + low threat
    if state.curiosity > 0.65 && state.threat < 0.45 {
        tags.push(ResonanceEntry { tag: ResonanceTag::Pull, intensity: state.curiosity });
    }

    // Tangle — moderate tension (uncertainty+threat combo)
    let tension = (state.uncertainty + state.threat) / 2.0;
    if tension > 0.55 {
        tags.push(ResonanceEntry { tag: ResonanceTag::Tangle, intensity: tension });
    }
    ...
    if state.logical_consistency > 0.8
        && state.honesty > 0.8
        && state.utility > 0.55
    {
        tags.push(ResonanceEntry {
    ...
    if state.curiosity > 0.75 && state.threat < 0.4 {
        tags.push(ResonanceEntry { tag: ResonanceTag::Bloom, intensity: state.curiosity });
    }

    // Hum — fallback, always present when nothing else fires
    if tags.is_empty() {
        tags.push(ResonanceEntry { tag: ResonanceTag::Hum, intensity: 0.2 });
    }
