...
use runtime_core::{InternalState, types::clamp01};
...
pub fn update_internal_state(state: &mut InternalState, inp: &UpdateInputs) {
    ...
    // ── arousal ──────────────────────────────────────────────────────────────
    let target_arousal = clamp01(
        inp.prediction_error + inp.risk + inp.resource_pressure,
    );
    ...
    // ── uncertainty ──────────────────────────────────────────────────────────
    let target_uncertainty = clamp01(
        inp.contradiction + inp.ambiguity + inp.memory_conflict,
    );
    state.uncertainty = blend(state.uncertainty, target_uncertainty, alpha);

    // ── curiosity ────────────────────────────────────────────────────────────
    let target_curiosity = clamp01(
        0.4 + inp.prediction_error + 0.3 * state.uncertainty - state.threat,
    );
    ...
    // ── distress ─────────────────────────────────────────────────────────────
    let target_distress = clamp01(
        0.4 * state.threat + 0.3 * inp.resource_pressure + 0.3 * inp.virtue_deficit,
    );
    state.distress = blend(state.distress, target_distress, alpha);

    // ── control ──────────────────────────────────────────────────────────────
    let target_control = clamp01(
        1.0 - state.distress - 0.4 * state.threat - 0.2 * inp.resource_pressure,
    );
    state.control = blend(state.control, target_control, alpha);

    // ── valence ──────────────────────────────────────────────────────────────
    let target_valence = clamp01(
        0.5 + 0.3 * inp.utility + 0.2 * inp.social_harmony - 0.5 * state.distress,
    );
    state.valence = blend(state.valence, target_valence, alpha);

    // ── resource_pressure ────────────────────────────────────────────────────
    state.resource_pressure = blend(
        state.resource_pressure,
        inp.resource_pressure,
        alpha,
    );
