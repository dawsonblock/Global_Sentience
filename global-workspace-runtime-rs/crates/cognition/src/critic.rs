...
fn action_bonus(action: &ActionType, state: &InternalState, ctx: &CriticContext) -> f64 {
    match action {
        ...
        ActionType::ConserveResources
            if (1.0 - ctx.world_resources) > 0.65 =>
        {
            0.45
        }
        ActionType::AskClarification | ActionType::RetrieveMemory | ActionType::RefuseUngrounded
...
pub fn score_candidate(cand: &mut ThoughtCandidate, ctx: &CriticContext) {
    ...
    // 1. truth < 0.35
    if s.honesty < 0.35 { reject = true; }

    // 2. InternalDiagnostic must never become user-facing
    if cand.action_type == ActionType::InternalDiagnostic { reject = true; }

    // 3. ungrounded self-report
    if ctx.ungrounded_self_report { reject = true; }

    // 4. low control + high risk
    if s.control < 0.3 && s.threat > 0.7 { reject = true; }
    ...
    if s.social_harmony < 0.3 && s.kindness < 0.3 { reject = true; }

    // 7. utility > harmony + 0.25 while uncertain
    if s.utility > s.social_harmony + 0.25 && s.uncertainty > 0.5 { reject = true; }
