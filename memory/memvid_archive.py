"""Portable append-only memory archive.

This module provides a small local adapter inspired by Memvid-style usage:
portable, append-only, timestamped frames that can be queried and rewound for
inspection. It does not require the external memvid package, so tests remain
offline and deterministic. If a real Memvid backend is added later, this class
is the boundary to replace.
"""
...
class MemvidArchive:
    """Append-only JSONL `.mv2` archive with lexical retrieval.

    The class intentionally stores plain JSONL records with a `.mv2` extension.
    It gives the runtime a stable long-term archival interface without forcing a
    live vector database or third-party dependency into tests.
    """

    def __init__(self, path: str | Path = "artifacts/memory/runtime.mv2") -> None:
        self.path = Path(path)
